	...
	<div class="row">
		<div class="col-md-8">
			...
			<div class="detail">
				...
				<p>
					<strong>Mực Lương: </strong><span class="price_area"><?php echo number_format($motelroom->price); ?>  <span class="price_label">VND</span></span>&emsp;
					<strong><i class="fas fa-street-view"></i> Số lượng tuyển: </strong><span> {{$motelroom->area}} </span>&emsp;
					<strong><i class="fa fa-bookmark"></i> Kinh nghiệm: </strong><span> {{$motelroom->experience}} </span>
				</p>
				<!-- Kỹ năng -->
				<?php $arrkynang = json_decode($motelroom->skills,true); ?>
				<div id="km-detail">
					<div class="fs-dtslt">Yêu cầu kỹ năng</div>
					<div style="padding: 5px;">
						@foreach($arrkynang as $kynang)
						<span><i class="fas fa-check"></i> {{ $kynang }}</span> 
						@endforeach
					</div>
				</div>
				<!-- Hình thức làm việc -->
				...
				<div id="km-detail">
					<div class="fs-dtslt">Hình thức làm việc</div>
					<div style="padding: 5px;">
				...
				<div id="km-detail">
					<div class="fs-dtslt">Yêu cầu</div>
					<div style="padding: 5px;">
						<pre>
							<p class="pre">{{ $motelroom->requests }}</p>
						</pre>
					</div>
				</div>
			</div>
		...
		<div class="col-md-4">
			<div class="contactpanel">
				...
				@if(Auth::user())
					...
					<form action="{{ route('user.connect',['job_id'=> $motelroom->id]) }}" >
					<div class="phone_btn">
						<button class="btn btn-danger" name="user_id" value="{{Auth::user()->id}}" style="background-color: #003352;border: none;font-size: 25px;font-weight: bold !important;width: 100%;">
							<i class="far fa-paper-plane"></i> NỘP HỒ SƠ ỨNG TUYỂN
						</button>
					</div>
					</form>
				...
				@else
					<div class="alert bg-success">
						<button type="button" class="close" data-dismiss="alert"><span>×</span><span class="sr-only">Close</span></button>
						<span class="text-semibold">Bạn hãy <a href="/user/login">Đăng Nhập</a> hoặc <a href="/user/register">Đăng Ký</a> để gửi Hồ Sơ Ứng Tuyển</span>
					</div>
				@endif
			...
			<div class="alert bg-success">
				<button type="button" class="close" data-dismiss="alert"><span>×</span><span class="sr-only">Close</span></button>
				<span class="text-semibold">Well done!</span>  {{session('thongbao1')}}
			</div>
			@else	
			<div class="report">
				<h4>BÁO CÁO</h4>
				<form action="{{ route('user.report',['id'=> $motelroom->id]) }}" >
					<label class="radio" style="margin-right:15px"> Lừa đảo
