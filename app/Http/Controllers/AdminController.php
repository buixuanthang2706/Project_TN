...
class AdminController extends Controller
{
    ...
    public function getListMotel(){
      ...
      return view('admin.job.list',['motelrooms'=>$motelrooms]);
    }
    public function ApproveMotelroom($id){
      $room = Motelroom::find($id);
      $room->approve = 1;
      $room->save();
      return redirect('admin/congviec/list')->with('thongbao','Đã kiểm duyệt bài đăng: '.$room->title);
    }
    public function UnApproveMotelroom($id){
      $room = Motelroom::find($id);
      $room->approve = 0;
      $room->save();
      return redirect('admin/congviec/list')->with('thongbao','Đã bỏ kiểm duyệt bài đăng: '.$room->title);
    }
    public function DelMotelroom($id){
      $room = Motelroom::find($id);
      $room->delete();
      return redirect('admin/congviec/list')->with('thongbao','Đã xóa bài đăng');
    }
