...
class MotelController extends Controller
{
	...
	public function userReport($id,Request $request){
	    ...
	    $report->save();
	    $getmotel = Motelroom::find($id);
		return redirect('congviec/'.$getmotel->slug)->with('thongbao1','Cảm ơn bạn đã báo cáo, đội ngũ chúng tôi sẽ xem xét');
	}
	public function UngTuyen($job_id,Request $request){
	    $connect = new Connect();
	    $connect->job_id = $job_id;
	    $connect->user_id = $request->user_id;
	    $connect->status = 0;
	    $connect->save();
	    $getmotel = Motelroom::find($job_id);
		return redirect('congviec/'.$getmotel->slug)->with('thongbao','Đã nộp hồ sơ!!! Hãy đợi nhà tuyển dụng phản hồi');
	}
}
